namespace Stubbl.Api.Core.QueryHandlers
{
   ...
   using Queries.ListTeamLogs.Version1;
   ...
   public class ListTeamLogsQueryHandler : IQueryHandler<ListTeamLogsQuery, ListTeamLogsProjection>
   {
      ...
      public async Task<ListTeamLogsProjection> HandleAsync(ListTeamLogsQuery query, CancellationToken cancellationToken)
      {
         ...
         if (query.StubIds != null)
         {
            ...
            {
               filter = Builders<Log>.Filter.And(filter, Builders<Log>.Filter.Where(l => l.StubIds.Contains(stubId)));
            }
         }
         ...
         var logs = await _logsCollection.Find(filter)
            ...
            (
               ...
               (
                  ...
                  l.Request.QueryStringParameters.Select(qcc => new QueryStringParameter(qcc.Key, qcc.Value)).ToList(),
                  l.Request.Body,
