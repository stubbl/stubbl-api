...
namespace Stubbl.Api.Core.Notifications.Email.TeamInvitation.Version1
{
   ...
   {
      public TeamInvitationEmail(string toEmailAddress, string teamName, string authenticatedMemberName, ObjectId invitationId)
      {
         ...
         {
            Text = $@"<p><strong>{authenticatedMemberName}</strong> has invited you to join Stubbl team '<strong><a href=""http://stubbl.it/invitations/{invitationId}"">{teamName}</a></strong>'.</p><p>Stubbl <3</p>"
         };
