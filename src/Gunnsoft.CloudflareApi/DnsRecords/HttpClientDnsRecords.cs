...
namespace Gunnsoft.CloudflareApi.DnsRecords
{
    public class HttpClientDnsRecords : IDnsRecords
    {
        ...
        public async Task<CreateDnsRecordResponse> CreateAsync(CreateDnsRecordRequest request,
        ...
        public async Task<ListDnsRecordsResponse> ListAsync(ListDnsRecordsRequest request,
            CancellationToken cancellationToken)
        {
            ...
            {
                pathAndQueryString +=
                    $"?{string.Join("&", queryString.AllKeys.Select(key => $"{WebUtility.UrlEncode(key)}={WebUtility.UrlEncode(queryString[key])}"))}";
            }
